...
<title>Implementation LFM Basique Avec Chevauchement entre Items</title>
</head>
<body>
<p>
<?php
...
$file = fopen("http://localhost/PFE/admin/Projection/matrice_Adj_Item.txt", "r");
$matrix_Adj = array();
$matrix = array();
$i = 0;
while(!feof($file)){
...
echo "<div id = 'titre'> <h1> Algorithme LFM basique à l'éxtension 'AVEC CHEVAUCHEMENT' Entre Items </h1> </div>";
//print_r($matrix);
...
$file_communities = fopen("communities_Item.txt", "w");
for($k = 0; $k<count($info[0]); $k++){
    $line = "";
    for($i = 0; $i< count($info[0][$k]); $i++){
        if($info[0][$k][$i]==1){
            $j = $i+1;
            $sql = "select IDitem from user where IDiprog = ".$j;
