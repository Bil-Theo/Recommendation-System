...
$user = fopen("datasets/u.user","r");

echo "<p font-weight= 'bold'>On commence</p>";
 while(!feof($user)){
     $line = explode("|",fgets($user));
     $requete = "insert into user values (".$line[0].",".$line[1].",\"".$line[2]."\",\"".$line[3]."\",".$line[4].");";
     echo $requete."<br>";
     $save = $dbh->prepare($requete);
     $save->execute();
 }
 echo "<p>Table user rempli avec succes<p/>";
fclose($user);
